...
<style>
    * {
    ...
    /* Main Grid Container jisme table scroll hogi */
    ...
    /* Webkit Scrollbar Design - Motai aur Smooth Look ke liye */
    ...
    .diary-grid-container::-webkit-scrollbar-thumb {
        ...
        /* Isse thumb floating aur modern lagta hai */
    }
...
<script>
    $(function() {
        ...
        let workingPlanExceptions = []; // <--- Yeh nayi line add karein
        ...
        function getCalendarDateRange() {
            if (currentView === 'week') {
                ...
                // Yahan weekDays[6] ki jagah last index (365th day) kar dein taaki poore saal ka data load ho sake
        ...
        window.loadCalendarData = function() {
            ...
            return App.Http.Calendar.getCalendarAppointmentsForTableView(startDate, endDate).done((response) => {
                ...
                workingPlanExceptions = response.working_plan_exceptions || []; // <--- Yeh line add karein
            });
        }
        ...
        function generateSlotsWithBreaks(dateStr, dayKey) {
            ...
            // 1. Check karein ki kya is specific date ke liye koi exception set hai
            let dayPlan = null;
            ...
            if (exception) {
                // Agar exception mil gaya aur start/end time hai toh wahi use hoga
                if (!exception.start_time || !exception.end_time) {
                    return slotList; // Agar office band hai toh khali list return karega
                }
                dayPlan = {
                ...
                // Agar exception nahi hai toh default weekly working plan use hoga
        ...
        function showAppointmentDetails(appointment) {
            const customerName = appointment.customer.first_name === appointment.customer.last_name ?
                appointment.customer.first_name : [appointment.customer.first_name, appointment.customer.last_name].filter(Boolean).join(' - ') || '-';
        ...
        window.renderCalendar = function() {
            ...
            if (currentView === 'week') {
                ...
                weekDays.forEach(d => {
                    // Update: passed dateAttr to generateSlotsWithBreaks
                ...
                uniqueTimeSlots.forEach((time, index) => {
                    ...
                    weekDays.forEach((d) => {
                        ...
                        const slot24Time = formatTimeSlotTo24(time);
                ...
                daySlots.forEach((slotObj) => {
                    ...
                    if (slotObj.isBreak) {
                        ...
                        const [yearVal, monthVal, dayVal] = dateAttr.split('-').map(Number);
                        const slot24Time = formatTimeSlotTo24(slotObj.time); 
                        const [hVal, minVal] = slot24Time.split(':').map(Number);
                        const slotDateTime = new Date(yearVal, monthVal - 1, dayVal, hVal, minVal);
                        const now = new Date();
                        const cutoffTime = new Date(now.getTime() - (5 * 60 * 60 * 1000));
