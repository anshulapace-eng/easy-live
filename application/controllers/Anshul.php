 ...
 * @package Controllers
 */
class Anshul extends EA_Controller
{
   ...
   public function save_appointment(): void
{
    try {
        ...
        if ($appointment_data) {
            ...
            // ------------------------------------------------------------------
            // 🛑 CUSTOM VALIDATION 1: 2 Weeks (14 Days) Advance Limit Check
            // ------------------------------------------------------------------
            $startDate = new DateTime($appointment['start_datetime']);
            
            // Aaj se exactly 14 din baad ki raat 11:59:59 PM tak allowed hai
            $maxBookingDate = new DateTime('now');
            $maxBookingDate->setTime(23, 59, 59);
            $maxBookingDate->modify('+14 days');

            if ($startDate > $maxBookingDate) {
                json_response([
                    'success' => false,
                    'message' => 'Appointments can only be scheduled up to 2 weeks (14 days) in advance.',
                ]);
                return;
            }

            // ------------------------------------------------------------------
            // 🛑 CUSTOM VALIDATION 2: Maximum 15 Minutes Duration Check
            // ------------------------------------------------------------------
            $startObj = new DateTime($appointment['start_datetime']);
            $endObj = new DateTime($appointment['end_datetime']);
            
            // Diff in minutes (Rounding off precision errors)
            $diffInMinutes = round(abs($endObj->getTimestamp() - $startObj->getTimestamp()) / 60);

            if ($diffInMinutes > 15) {
                json_response([
                    'success' => false,
                    'message' => 'The appointment duration cannot be more than 15 minutes. Selected duration: ' . $diffInMinutes . ' mins.',
                ]);
                return;
            }
    ...
    /**
     * Update appointment check-in / check-out status.
     */
    /**
     * Update appointment check-in / check-out status.
     */
