...
class Appointments_model extends EA_Model
{
    ...
    public function api_decode(array &$appointment, ?array $base = null): void
    {
        ...
        // YAHAN DO CHEEZEIN ADD KI GAYI HAIN TAKI KISI BHI NAAM SE AAYE TO PICK HO JAYE
        if (array_key_exists('appointmentType', $appointment)) {
            $decoded_resource['appointment_type'] = $appointment['appointmentType'];
        }
        
        if (array_key_exists('appointment_type', $appointment)) {
            $decoded_resource['appointment_type'] = $appointment['appointment_type'];
        }
