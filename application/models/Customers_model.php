...
class Customers_model extends EA_Model
{
    ...
    /**
     * Normalize a phone number so formatting differences do not create false duplicates.
     *
     * @param string|null $phone_number The phone number to normalize.
     *
     * @return string Returns the normalized phone number.
     */
    protected function normalize_phone_number(?string $phone_number): string
    {
    ...
    /**
     * Check whether a phone number is already used by another customer.
     *
     * @param string|null $phone_number The phone number to look for.
     * @param int|null $customer_id The current customer ID if updating an existing customer.
     *
     * @return bool Returns true if the phone number is already used by another customer.
     */
    protected function phone_number_exists(?string $phone_number, ?int $customer_id = null): bool
    {
        $normalized_phone_number = $this->normalize_phone_number($phone_number);
        ...
        foreach ($customers as $customer) {
            if ($this->normalize_phone_number($customer['phone_number'] ?? '') === $normalized_phone_number) {
                return true;
            }
        }
    ...
    public function validate(array $customer): void
    {
        ...
        //     if ($this->phone_number_exists($customer['phone_number'], $customer_id)) {
        //         throw new InvalidArgumentException(
        //             'The provided phone number is already in use, please use a different one.',
        //         );
        //     }
        // }
    }
